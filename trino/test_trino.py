...
class PrestoTestCase(DataprocTestCase):
    ...
    def verify_instance(self, name, coordinators, workers):
        ...
        self.__verify_schema_via_trino(name, schema)

        self.__create_table(name, table, schema)
        self.__insert_data_into_table_via_hive(name, table, schema)
        self.__validate_data_in_table_via_trino(name, table, schema)
    ...
    def __verify_schema_via_trino(self, name, schema):
        query = "show schemas;"
        _, stdout, _ = self.assert_instance_command(
            name, "trino --catalog=hive --execute='{}' --output-format TSV".
            format(query))
    ...
    def __validate_data_in_table_via_trino(self, name, table, schema):
        query = "SELECT number, count(*) AS total FROM {} GROUP BY number ORDER BY number DESC;".format(
            table)
        _, stdout, _ = self.assert_instance_command(
            name,
            "trino --catalog=hive --schema={} --execute='{}' --output-format TSV"
            .format(schema, query))
    ...
    def __verify_coordinators_count(self, name, coordinators, server_param=""):
        ...
        _, stdout, _ = self.assert_instance_command(
            name, "trino {} --execute '{}' --output-format TSV".format(
                server_param, query))
        self.assertEqual(
    ...
    def test_trino(self, configuration, machine_suffixes, coordinators,
                    workers):
    ...
    def test_trino_custom_port(self, configuration, machine_suffixes,
                                coordinators, workers):
        if self.getImageOs() == 'centos':
            self.skipTest("Not supported in CentOS-based images")
